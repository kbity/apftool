...
def decode(apf: str | bytes, format: str = 'PNG', returnImageObject: bool = False):
    if type(apf) == bytes:
...
def encode(img: bytes | Image.Image, lineskip: int = 1, findbestlineskip: bool = False, returnbytes: bool = False):
    if type(img) == bytes:
    ...
    for num in runlens:
        output.append(chr(num+32))

    stroutput = "".join(output)
    apflist.append(stroutput)
    apftext = "\n".join(apflist)
    if returnbytes:
