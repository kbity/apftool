...
async function load_canvas_from_attr() {
    ...
    for (const canvas of canvases) {
        ...
        // parse delay from metadata
        const apf_lines = text
            .split(/\r?\n/)
            .filter(line => line);

        const metadata = apf_lines[1]
            .trim()
            .split(",");
        ...
        const imageFrames = frames.map(bitmap_to_imagedata);

        canvas.width = imageFrames[0].width;
        canvas.height = imageFrames[0].height;

        const ctx = canvas.getContext("2d");

        // single frame: render once
        if (imageFrames.length === 1) {
            ctx.putImageData(imageFrames[0], 0, 0);
            continue;
        }
